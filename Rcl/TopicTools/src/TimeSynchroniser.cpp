...
void TimeSynchroniser::appendSubscriber(QQmlListProperty<QObject>* _property, QObject* _subscriber)
{
  ...
  connect(_subscriber, SIGNAL(messageReceived(const QVariant&, quint64, const QString&)),
          self, SLOT(handleMessage(const QVariant&)));
  self->m_subscribers.append(_subscriber);
}

int TimeSynchroniser::subscriberCount(QQmlListProperty<QObject>* _property)
{
  TimeSynchroniser* self = qobject_cast<TimeSynchroniser*>(_property->object);
  QMutexLocker l(&self->m_mutex);
...
QObject* TimeSynchroniser::subscriber(QQmlListProperty<QObject>* _property, int _index)
{
  TimeSynchroniser* self = qobject_cast<TimeSynchroniser*>(_property->object);
  QMutexLocker l(&self->m_mutex);
...
void TimeSynchroniser::clearSubscribers(QQmlListProperty<QObject>* _property)
{
  ...
  {
    s->setParent(nullptr);
    disconnect(s, SIGNAL(messageReceived(const QVariant&, quint64, const QString&)),
            self, SLOT(handleMessage(const QVariant&)));
  }
...
QQmlListProperty<QObject> TimeSynchroniser::subscribers()
{
  return QQmlListProperty<QObject>(this, this,
          &TimeSynchroniser::appendSubscriber,
          &TimeSynchroniser::subscriberCount,
          &TimeSynchroniser::subscriber,
          &TimeSynchroniser::clearSubscribers);
}
...
struct ros_time
{
  ...
  bool operator!=(const ros_time& _rhs)
  {
    return sec != _rhs.sec or nanosec != _rhs.nanosec;
  }
  bool operator==(const ros_time& _rhs)
  {
    return sec == _rhs.sec and nanosec == _rhs.nanosec;
  }
  static ros_time fromMsg(const QVariant& _message)
  {
    return ros_time(_message.toMap()["header"]);
  }
...
void TimeSynchroniser::handleMessage(const QVariant& _message)
{
  ...
  while(true)
  {
    ...
    {
      if(m_messages[s].empty()) return; // <- no message received yet
      ros_time ct = ros_time::fromMsg(m_messages[s].first());
      if(ct < earliest_time) earliest_time = ct;
    }
