...
Subscriber::Subscriber(QObject* _parent)
    : RosObject(_parent), m_subscription(rcl_get_zero_initialized_subscription()), m_skip(0),
      m_skipCount(0)
{
  ...
  if(m_message_definition)
  {
    ...
    case RCL_RET_OK:
    {
      ...
      else
      {
        ...
        emit(messageReceived(m_lastMessage, RosThread::instance()->now(),
                             QByteArray((const char*)message_info.publisher_gid.data).toHex()));
      }
      break;
    }
    case RCL_RET_SUBSCRIPTION_TAKE_FAILED:
      ...
      break;
    default:
      qWarning() << "Failed to get subscribtion message" << m_topic_name
...
void Subscriber::subscribe()
{
  ...
  if(not m_data_type.isEmpty() and not m_topic_name.isEmpty())
  {
    ...
    if(m_message_definition)
    {
      ...
      if(rcl_subscription_init(&m_subscription, RosThread::instance()->rclNode(),
                               m_message_definition->typeSupport(), qPrintable(m_topic_name),
                               &subscription_ops)
         != RCL_RET_OK)
      {
