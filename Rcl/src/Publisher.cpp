...
Publisher::Publisher(QObject* _parent)
    : RosObject(_parent), m_publisher(rcl_get_zero_initialized_publisher())
{
}
...
void Publisher::start_publisher()
{
  ...
  if(not m_data_type.isEmpty() and not m_topic_name.isEmpty())
  {
    ...
    rcl_publisher_options_t publisher_ops
      = rcl_publisher_get_default_options(); // TODO support for QoS
    if(rcl_publisher_init(&m_publisher, RosThread::instance()->rclNode(),
